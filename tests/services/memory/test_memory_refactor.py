...
def _ev(eid, run_id="r1", src=EvidenceSource.GOOGLE_SEARCH, rel=0.8, title="Test", snippet="test snippet", ref="ref"):
    return EvidenceItem(
        evidence_id=eid,
        run_id=run_id,
        source_type=src,
...
def test_promotion_rejects_below_threshold():
    result = PromotionGate(EvidenceStore(), min_relevance=0.5).promote_batch("r1", [_ev("e1", rel=0.3)], phase="custom")
...
def test_contradiction_emits_trace():
    ...
    store.add(
        "r1", _ev("e1", run_id="r1", src=EvidenceSource.GOOGLE_SEARCH, title="employees", snippet="5000 employees")
    )
    PromotionGate(store).promote_batch(
        "r1", [_ev("e2", run_id="r1", src=EvidenceSource.USER_PROVIDED, title="employees", snippet="200 employees")]
    )
