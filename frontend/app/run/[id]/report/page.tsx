...
import { getRun, getReport, approveReport, requestDeeperInvestigation } from '@/lib/api'
import ReasoningChain from '@/components/ReasoningChain'
import ConfidenceNarrative from '@/components/ConfidenceNarrative'
import Badge from '@/components/ui/Badge'
import Spinner from '@/components/ui/Spinner'
import type { Run, AdaptiveReport, ReportOpportunity } from '@/lib/types'
...
function OpportunityCard({ opp }: { opp: ReportOpportunity }) {
  const [evidenceOpen, setEvidenceOpen] = useState(false)
  const tier = opp.tier.toLowerCase()
  const pct = Math.round(opp.confidence * 100)
  return (
    <div className={`bg-canvas-raised border border-edge-subtle rounded-md p-5 border-l-[3px] ${TIER_BORDER[tier] ?? 'border-l-edge'} print:break-inside-avoid print:bg-white print:border-gray-300`}>
    ...
    <div className="min-h-screen bg-canvas print:bg-white">
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 pb-24 print:p-0 print:max-w-none">
        ...
        <section className="mb-8">
          <p className="text-xs text-ink-secondary uppercase tracking-wider font-medium mb-3 print:text-black">Executive Summary</p>
        ...
        <section className="mb-8">
          ...
          <div className="space-y-4">{sorted.map((opp, i) => <OpportunityCard key={i} opp={opp} />)}</div>
        </section>
        ...
        {report.what_we_dont_know.length > 0 && (
          <section className="mb-8 bg-amber/10 border border-amber/20 rounded-md p-5 print:bg-white print:border-gray-300">
            <p className="text-xs text-amber uppercase tracking-wider font-medium mb-3 print:text-black">What We Don&apos;t Know</p>
        ...
        <section className="mb-8">
          <SectionHeader>Confidence Assessment</SectionHeader>
