      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-sm text-ink-secondary">
            Add what you learned from surveys, interviews, and stakeholder conversations. The system
            will re-evaluate affected hypotheses and update the report.
          </p>
        </div>
          ...
          <div className="bg-mint/5 border border-mint/20 rounded-md p-5 mb-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-sm text-ink-secondary">
                {result.evidence_added} evidence added, {result.hypotheses_affected} hypotheses
                re-evaluated
              </span>
            </div>
              ...
              <div className="space-y-2">
                ...
                {result.deltas.map((d) => {
                  ...
                  return (
                    <div
                      key={d.hypothesis_id}
                      className="bg-canvas-raised border border-edge-subtle rounded-md p-3"
                    >
                      ...
                      <div className="flex items-center gap-4 mt-1.5 text-2xs font-mono">
                        <span className="text-ink-tertiary">
                          {Math.round(d.confidence_before * 100)}%
                        </span>
                        <span
                          className={
                            changed ? (up ? "text-mint" : "text-rose") : "text-ink-tertiary"
                          }
                        >
                        ...
                        {statusChanged && (
                          <Badge
                            variant={
                              d.status_after === "validated"
                                ? "mint"
                                : d.status_after === "rejected"
                                  ? "rose"
                                  : "amber"
                            }
                          >
