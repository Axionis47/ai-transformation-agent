...
export default function OpportunityRow({ opportunity, evidence }: OpportunityRowProps) {
  ...
  const avgRelevance =
    matched.length > 0 ? matched.reduce((s, e) => s + e.relevance_score, 0) / matched.length : 0;
  const sourceTypes = new Set(matched.map((e) => e.source_type));

  return (
    <div
      className={`border-l-[3px] ${borderColor} pl-3 py-3 bg-surface border border-l-[3px] border-border mb-2`}
    >
      <div className="flex justify-between items-start mb-1">
        <p className="text-text-primary" style={{ fontSize: "14px", fontWeight: 500 }}>
          {opportunity.name}
        </p>
        <span className={`font-mono uppercase ${textColor}`} style={{ fontSize: "12px" }}>
          {opportunity.tier}
        </span>
      </div>
      <p className="text-text-muted font-mono mb-2" style={{ fontSize: "12px" }}>
      ...
      <div className="flex gap-3">
        <span className="text-text-muted font-mono" style={{ fontSize: "12px" }}>
          Evidence:{" "}
          {opportunity.evidence_ids.map((id) => (
            <a key={id} href={`#${id}`} className="text-accent cursor-pointer hover:underline mr-1">
              [{id.slice(0, 8)}]
            </a>
          ))}
        </span>
        <span className="text-text-muted font-mono ml-3" style={{ fontSize: "12px" }}>
          {matched.length} items, {sourceTypes.size} source{sourceTypes.size !== 1 ? "s" : ""}, avg
          rel {avgRelevance.toFixed(2)}
        </span>
      </div>
      {opportunity.adaptation_needed && (
        <p className="text-text-muted font-mono mt-1" style={{ fontSize: "12px" }}>
          Adaptation: {opportunity.adaptation_needed}
        </p>
      )}
      {opportunity.risks && opportunity.risks.length > 0 && (
        <div className="mt-1">
          ...
          {opportunity.risks.map((r, i) => (
            <span key={i} className="text-text-muted font-mono" style={{ fontSize: "12px" }}>
              {r}
              {i < opportunity.risks.length - 1 ? " · " : ""}
            </span>
          ))}
        </div>
      )}
