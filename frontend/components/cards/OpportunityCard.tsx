...
export default function OpportunityCard({ opportunity, evidence }: OpportunityCardProps) {
  ...
  return (
    <div
      className={`bg-canvas-raised border border-edge-subtle border-l-[3px] ${border} rounded-md overflow-hidden`}
    >
      <div className="px-5 py-4 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="grid grid-cols-4 gap-4 mt-4">
          {opportunity.feasibility > 0 ? (
            ...
            <div>
              <span className="text-2xs text-ink-tertiary uppercase">Feasib.</span>
              <span className="text-2xs text-ink-tertiary font-mono ml-2">—</span>
            </div>
          )}
          {opportunity.roi > 0 ? (
            ...
            <div>
              <span className="text-2xs text-ink-tertiary uppercase">ROI</span>
              <span className="text-2xs text-ink-tertiary font-mono ml-2">—</span>
            </div>
          )}
          {opportunity.time_to_value > 0 ? (
            ...
            <div>
              <span className="text-2xs text-ink-tertiary uppercase">TTV</span>
              <span className="text-2xs text-ink-tertiary font-mono ml-2">—</span>
            </div>
          )}
          {opportunity.confidence > 0 ? (
            ...
            <div>
              <span className="text-2xs text-ink-tertiary uppercase">Confidence</span>
              <span className="text-2xs text-ink-tertiary font-mono ml-2">—</span>
            </div>
          )}
        </div>
        {opportunity.data_sufficiency === "insufficient_data" && (
          <div className="mt-2">
            <span className="text-2xs font-mono bg-amber/10 text-amber px-2 py-0.5 rounded uppercase">
              Insufficient Data
            </span>
          </div>
        )}
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {opportunity.risks.map((r, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-2 text-sm text-ink-secondary leading-relaxed"
                  >
                    <span
                      className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${tier === "easy" ? "bg-mint/50" : tier === "medium" ? "bg-amber/50" : "bg-rose/50"}`}
                    />
