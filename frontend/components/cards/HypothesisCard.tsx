...
const STATUS_BADGE: Record<
  HypothesisStatus,
  { variant: "indigo" | "amber" | "mint" | "rose" | "muted"; label: string }
> = {
...
const STATUS_BORDER: Record<HypothesisStatus, string> = {
  forming: "border-l-indigo",
  testing: "border-l-amber",
  validated: "border-l-mint",
  rejected: "border-l-rose",
...
export default function HypothesisCard({ hypothesis }: HypothesisCardProps) {
  ...
  return (
    <div
      className={`bg-canvas-raised border border-edge-subtle border-l-[3px] ${STATUS_BORDER[status]} rounded-md overflow-hidden ${isRejected ? "opacity-60" : ""}`}
    >
      <div className="px-5 py-4 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-start justify-between gap-4">
          <h3
            className={`text-base font-semibold text-ink leading-snug ${isRejected ? "line-through" : ""}`}
          >
        ...
        <div className="flex items-center gap-4 mt-3 text-2xs font-mono">
          ...
          {(evidence_conditions?.length ?? 0) > 0 && (
            <span className="text-amber">{evidence_conditions.length} conditions</span>
          )}
      ...
      >
        <div className="overflow-hidden">
          <div className="px-5 pb-5 border-t border-edge-subtle pt-4 space-y-3">
            {hypothesis.reasoning_chain.length > 0 && (
              <div>
                <p className="text-2xs text-ink-tertiary uppercase tracking-wider mb-2 font-medium">
                  Reasoning
                </p>
                <ul className="space-y-1">
                  {hypothesis.reasoning_chain.map((s, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-ink-secondary">
                      <span className="mt-1 w-1.5 h-1.5 rounded-full bg-indigo/50 shrink-0" />
                      {s.description}
                    </li>
                  ))}
                </ul>
            ...
            {hypothesis.test_results.length > 0 && (
              <div>
                ...
                <ul className="space-y-1">
                  {hypothesis.test_results.map((t, i) => (
                    <li key={i} className="text-sm text-ink-secondary">
                      <span className="font-mono text-2xs text-amber mr-2">{t.test_type}</span>
                      {t.finding}
                    </li>
                  ))}
                </ul>
            ...
            {hypothesis.risks.length > 0 && (
              <div>
                <p className="text-2xs text-rose uppercase tracking-wider mb-2 font-medium">
                  Risks
                </p>
                <ul className="space-y-1">
                  {hypothesis.risks.map((r, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-ink-secondary">
                      <span className="mt-1 w-1.5 h-1.5 rounded-full bg-rose/50 shrink-0" />
                      {r}
                    </li>
                  ))}
                </ul>
