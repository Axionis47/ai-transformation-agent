      ...
      {editing ? (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={save} className="text-2xs font-medium text-mint hover:text-mint-bright transition-colors">Save</button>
            <button onClick={() => { setEditing(false); setValue(assumption.value) }} className="text-2xs font-medium text-ink-tertiary hover:text-ink-secondary transition-colors">Cancel</button>
          </div>
        </div>
      ) : (
        <div className="group">
