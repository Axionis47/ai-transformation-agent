...
export default function Header({ run, agents = [] }: HeaderProps) {
  ...
  const toolCalls =
    (budget_state.external_search_queries_used || 0) + (budget_state.rag_queries_used || 0);
  ...
  return (
    <header className="h-12 bg-canvas-raised border-b border-edge-subtle sticky top-0 z-30 flex items-center px-6 gap-5">
      ...
      <Badge variant={STATUS_VARIANT[s] || "muted"}>{statusLabel(status)}</Badge>
        ...
        {agentCount > 0 && (
          <>
            ...
            <span className="tabular">
              ...
              {runningAgents > 0 && (
                <span className="text-mint ml-1">({runningAgents} active)</span>
              )}
            </span>
          </>
        )}
