  ...
  return (
    <section id="evidence-annex" className="mb-8">
      <button onClick={() => setOpen(!open)}
        ...
        <span className={`transition-transform text-[10px] text-ink-tertiary ${open ? 'rotate-90' : ''}`}>&#9654;</span>
      ...
      {open && (
        <div className="mt-4 space-y-3">
          ...
          {filtered.map(ev => {
            const src = SOURCE_BADGE[ev.source_type] ?? { label: ev.source_type, variant: 'muted' as const }
            const isHighlighted = highlightId === ev.evidence_id
            return (
              <div key={ev.evidence_id} id={ev.evidence_id}
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="font-mono text-2xs text-ink-tertiary">{ev.evidence_id.slice(0, 12)}</span>
                  <Badge variant={src.variant}>{src.label}</Badge>
                  {ev.source_ref && <span className="text-2xs text-ink-tertiary truncate max-w-[200px]">{ev.source_ref}</span>}
                </div>
                <p className="text-sm font-semibold text-ink">{ev.title}</p>
                <p className="text-2xs text-ink-secondary line-clamp-2 mt-1 max-w-prose">&ldquo;{ev.snippet}&rdquo;</p>
                <div className="mt-2 max-w-xs">
                  <ScoreBar value={ev.relevance_score} label="Relevance" color="mint" />
                </div>
                {((ev as Record<string, unknown>).dimension || (ev as Record<string, unknown>).process_area) && (
                ...
                {ev.uri && (
                  <a href={ev.uri} target="_blank" rel="noopener noreferrer"
          ...
          {filtered.length === 0 && (
            <p className="text-2xs text-ink-tertiary font-mono py-4 text-center">No evidence for this filter.</p>
          )}
        </div>
      )}
    </section>
