  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        {running.length > 0 && <Badge variant="mint">{running.length} active</Badge>}
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  <span
                    className={`w-2 h-2 rounded-full shrink-0 ${statusIndicator(agent.status)}`}
                  />
                  <Badge variant={TYPE_VARIANT[agent.agent_type] ?? "muted"}>
                    {agent.agent_type}
                  </Badge>
                </div>
