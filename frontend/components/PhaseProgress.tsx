      ...
      {phases.map((phase, idx) => {
        ...
        return (
          <div key={phase} className="flex items-center">
            {idx > 0 && (
              <div className={`w-8 h-px shrink-0 ${isComplete ? 'bg-mint' : isCurrent ? 'bg-mint/50' : 'bg-edge-subtle'}`} />
            )}
            <div className="flex flex-col items-center gap-1.5 shrink-0">
              <div className={`relative w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                isComplete ? 'bg-mint border-mint' :
                isCurrent ? 'border-mint bg-transparent' :
                ...
                {isCurrent && <span className="absolute inset-0 rounded-full bg-mint/30 animate-pulse" />}
              </div>
              <span className={`text-center font-mono leading-none whitespace-nowrap ${
                isCurrent ? 'text-mint font-semibold' :
                isComplete ? 'text-ink-secondary' :
                'text-ink-tertiary'
