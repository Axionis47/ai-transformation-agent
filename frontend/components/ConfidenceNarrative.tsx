        ...
        {confidence > 0 && (
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-ink-tertiary font-mono">{Math.round(confidence * 100)}%</span>
            </div>
            <div className="w-full h-1.5 bg-edge-subtle rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all ${barColor}`} style={{ width: `${Math.round(confidence * 100)}%` }} />
            </div>
          </div>
        )}
