...
function stepColor(text: string): string {
  ...
  if (lower.includes("evidence") || lower.includes("found") || lower.includes("source"))
    return "border-blue-300 bg-blue-50";
  if (lower.includes("hypothesis") || lower.includes("theory") || lower.includes("assume"))
    return "border-purple-300 bg-purple-50";
  if (lower.includes("validated") || lower.includes("confirmed") || lower.includes("supports"))
    return "border-green-300 bg-green-50";
  if (lower.includes("rejected") || lower.includes("contradicts") || lower.includes("disproved"))
          ...
          <div className="space-y-3">
            {steps.map((step, i) => (
              <div key={i} className="flex items-start gap-3 relative">
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 z-10 ${dotColor(step)}`}
                >
                  {i + 1}
                </div>
