...
export default function ReasoningStage({
  state,
  depthBudget,
  onAnswer,
  loading,
  ...
  if (!state) {
    return loading ? (
      <div className="flex items-center gap-3 py-4">
        <Spinner />
        <span className="text-sm text-ink-secondary">Starting reasoning loop...</span>
      </div>
    ) : null;
  }
    ...
    <div className="space-y-5">
          ...
          <div key={i} className="flex items-center gap-1.5">
            <div
              className={`w-8 h-1.5 rounded-full ${
                i < current_loop
                  ? "bg-mint"
                  : i === current_loop && !completed
                    ? "bg-mint/40 animate-pulse-slow"
                    : "bg-edge-subtle"
              }`}
            />
      ...
      <div className="bg-canvas-raised border border-edge-subtle rounded-md p-4">
        ...
        <p className="text-2xs text-ink-tertiary mb-3">
          How well the system understands this company across all required fields
        </p>
        <ScoreBar
          value={overall_confidence}
          color={overall_confidence > 0.6 ? "mint" : overall_confidence > 0.3 ? "amber" : "rose"}
          showValue={false}
        />
      </div>
        ...
        <div className="bg-canvas-overlay border border-amber/20 rounded-md p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-2xs text-ink-tertiary">
              You can skip this and proceed to synthesis
            </span>
          </div>
          {escalation_fields && escalation_fields.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {escalation_fields.map((f) => (
                <span
                  key={f}
                  className="text-2xs font-mono bg-amber/10 text-amber px-2 py-0.5 rounded"
                >
                  {f.replace(/_/g, " ")}
                </span>
              ))}
          ...
          {contradictions && contradictions.length > 0 && (
            <div className="mb-3 text-sm text-ink-secondary border-l-2 border-amber/40 pl-3">
              <p className="text-2xs text-amber uppercase tracking-wider mb-1 font-medium">
                Conflicting Data
              </p>
      ...
      {pending_question && !completed && !isEscalation && (
        <div className="bg-canvas-overlay border border-mint/30 rounded-md p-5">
          <p className="text-2xs text-mint uppercase tracking-wider mb-2 font-medium">
            Agent Question
          </p>
          ...
          {pending_question.context && (
            <p className="text-sm text-ink-secondary mb-3 border-l-2 border-edge pl-3 italic">
              {pending_question.context}
            </p>
          )}
          <form onSubmit={handleSubmit} className="flex gap-2">
      ...
      {loading && (
        <div className="flex items-center gap-3">
          <Spinner />
          <span className="text-sm text-ink-secondary">Reasoning...</span>
        </div>
      )}
    </div>
  );
