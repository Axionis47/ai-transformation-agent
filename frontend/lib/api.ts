...
export async function submitIntake(runId: string, intake: CompanyIntake): Promise<Run> {
...
export async function confirmAssumptions(runId: string, body?: AssumptionsDraft): Promise<Run> {
...
export async function getAgentStates(runId: string): Promise<AgentState[]> {
  const res = await apiFetch<{ agents: AgentState[]; count: number }>(`/runs/${runId}/agents`);
  return res.agents;
}
...
export async function getHypothesis(runId: string, hypothesisId: string): Promise<Hypothesis> {
  return apiFetch<Hypothesis>(`/runs/${runId}/hypotheses/${hypothesisId}`);
}

export async function getInteractions(runId: string): Promise<UserInteractionPoint[]> {
...
export async function respondToInteraction(
  ...
  return apiFetch<UserInteractionPoint>(`/runs/${runId}/interactions/${interactionId}/respond`, {
    method: "POST",
    body: JSON.stringify({
