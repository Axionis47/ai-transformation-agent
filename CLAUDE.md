...
## Pipeline stages — 5 stages in sequence
...
### Stage 2: Assumptions Draft
- Grounder: Gemini + Google Search grounding learns about the company
- Output: list of Assumption objects with confidence and source
- User reviews, confirms, or edits via frontend
- Emits: ASSUMPTIONS_DRAFT_CREATED, ASSUMPTIONS_CONFIRMED

### Stage 3: Thought Engine (reasoning loop)
- Bounded by depth_budget (default 3 loops from config/defaults.yaml)
- Each loop: assess what is known, run MID (Missing Information Detection), decide action
- MID compares current evidence against required fields, identifies biggest gaps
- Actions available per loop: query RAG, call grounder, ask user a question
- User interaction: when MID says info is missing and tools cannot fill it, emit USER_QUESTION_ASKED
- Evidence accumulates across loops — loop 3 has more evidence than loop 1
- Stop conditions: confidence_threshold exceeded OR depth_budget exhausted OR all required fields covered
- Emits: REASONING_LOOP_STARTED, REASONING_LOOP_COMPLETED, MID_GAP_DETECTED, USER_QUESTION_ASKED

### Stage 4: Pitch Synthesis
- Maps accumulated evidence to opportunity templates
- 3-tier output:
  - Easy: direct match to past engagement, high confidence, proven ROI
  - Medium: similar engagement exists, different context, needs adaptation explicitly stated
  - Hard: novel application, low evidence, high potential, uncertainty flagged
- ROI translated from matched past engagements, adjusted for company size and industry
- Feasibility, ROI, time-to-value, confidence scored separately — all explainable
- Emits: OPPORTUNITIES_COMPUTED, ROI_MODEL_COMPUTED, CONFIDENCE_COMPUTED

### Stage 5: Report
- Operator brief: one readable page
- Evidence annex: all citations with source_type, uri, snippet
- Coverage gaps: fields not determined during reasoning
- Emits: REPORT_RENDERED, RUN_PUBLISHED
...
POST /v1/runs
  ...
  Returns: Run object (run_id, status=created, budgets, config_snapshot)
...
GET /v1/runs/{run_id}/ui
  Returns: UIHints object (stage-aware: actions, editable_fields, budget_view)
...
GET /v1/runs/{run_id}/trace
  Returns: list of TraceEvent for the run
...
```
PUT  /v1/runs/{run_id}/assumptions-final    # Sprint 4
POST /v1/runs/{run_id}/start                # Sprint 4 (triggers reasoning loop)
POST /v1/runs/{run_id}/answer               # Sprint 4 (user answers MID question)
POST /v1/runs/{run_id}/publish              # Sprint 5
GET  /v1/runs/{run_id}/evidence             # Sprint 5
GET  /v1/runs/{run_id}/metrics              # Sprint 7
```
...
```
Run                 — top-level run record, holds all state
RunStatus           — CREATED | INTAKE | ASSUMPTIONS_DRAFT | ASSUMPTIONS_CONFIRMED
                      | REASONING | SYNTHESIS | REPORT | PUBLISHED | FAILED
...
EvidenceItem        — evidence_id, run_id, source_type, source_ref, title, uri, snippet,
                      relevance_score, confidence_score, retrieval_meta
...
All 25 event types are in `core/events.py`. Do not define new event types outside that file.
...
RAG (Sprint 2):    RAG_QUERY_EXECUTED, RAG_RESULTS_FILTERED
Grounding (S3):    GROUNDING_CALL_REQUESTED, GROUNDING_CALL_COMPLETED,
                   GROUNDING_QUERIES_COUNTED, EXTERNAL_BUDGET_EXHAUSTED
Reasoning (S4):    REASONING_LOOP_STARTED, REASONING_LOOP_COMPLETED,
                   MID_GAP_DETECTED, USER_QUESTION_ASKED, USER_ANSWER_RECEIVED
Synthesis (S5):    OPPORTUNITIES_COMPUTED, ROI_MODEL_COMPUTED,
...
```yaml
budgets:
  external_search_query_budget: 5    # max Google Search queries (counted from grounding metadata)
  external_search_max_calls: 3       # defense-in-depth cap on grounding API calls
